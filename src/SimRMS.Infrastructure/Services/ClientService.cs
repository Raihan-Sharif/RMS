...
public class ClientService : IClientService
{
    ...
    public async Task<ClientDto> CreateClientAsync(CreateClientRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var existingClient = await GetClientByIdAsync(request.GCIF, cancellationToken);
        if (existingClient != null)
        {
            _logger.LogWarning("Client with GCIF {GCIF} already exists", request.GCIF);
            throw new ValidationException($"Client with GCIF '{request.GCIF}' already exists");
        }
        ...
        try
        {
            ...
            if (rowsAffected <= 0)
            {
                _logger.LogWarning("Client creation failed - No rows affected for GCIF: {GCIF}", request.GCIF);
                throw new DomainException("Failed to create client - no rows affected");
            }

            await _unitOfWork.CommitTransactionAsync(cancellationToken);

            _logger.LogInformation("Successfully created Client with GCIF: {GCIF}", request.GCIF);

            // Return the created client
            var createdClient = await GetClientByIdAsync(request.GCIF, cancellationToken);
