...
public interface IGenericRepository
{
    ...
    Task<T?> QuerySingleAsync<T>(string sqlOrSp, List<LB_DALParam>? parameters = null, bool isStoredProcedure = false, CancellationToken cancellationToken = default) where T : class;
    Task<IEnumerable<T>> QueryAsync<T>(string sqlOrSp, List<LB_DALParam>? parameters = null, bool isStoredProcedure = false, CancellationToken cancellationToken = default) where T : class;

    // Enhanced pagination with SP support
    Task<PagedResult<T>> QueryPagedAsync<T>(string sqlOrSp, List<LB_DALParam>? parameters = null, string? orderBy = null, bool isStoredProcedure = false, CancellationToken cancellationToken = default) where T : class;

    // Command operations (INSERT/UPDATE/DELETE)
    Task<int> ExecuteAsync(string sqlOrSp, List<LB_DALParam>? parameters = null, bool isStoredProcedure = false, CancellationToken cancellationToken = default);
    Task<T> ExecuteScalarAsync<T>(string sqlOrSp, List<LB_DALParam>? parameters = null, bool isStoredProcedure = false, CancellationToken cancellationToken = default);
    Task<ExecuteResult> ExecuteWithOutputAsync(string storedProcedure, List<LB_DALParam>? parameters = null, CancellationToken cancellationToken = default);

    // Bulk operations
    Task<int> ExecuteBatchAsync(IEnumerable<(string sql, List<LB_DALParam>? parameters)> commands, CancellationToken cancellationToken = default);
